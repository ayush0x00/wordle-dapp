module wordle4::test4{
    ...
    struct Wordle has key,copy,drop,store {
        ...
        id:u64,
        length:u64,
        states_formed:SimpleMap<u8,vector<u8>> //attempt:provided_input
        curr_state_res:SimpleMap<u8,vector<u8>>
    }
    ...
    public entry fun get_next_state_byIdx(account:&signer,_wordleId:u64, _attempt:u8, curr_state:u8, curr_idx:u8) acquires IdToWordle{
        let w_map = &mut borrow_global_mut<IdToWordle>(signer::address_of(account)).idWordleMap;
        let wordle = simple_map::borrow_mut(w_map,&_wordleId);
